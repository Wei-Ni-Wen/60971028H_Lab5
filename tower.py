...
# initialization
pygame.init()
image_tower = pygame.image.load(os.path.join("images", "rapid_test.png"))  # import tower's image

class Tower:
    def __init__(self, x, y):
        self.image = pygame.transform.scale(image_tower, (70, 70))  # # set image_tower's size
...
class TowerGroup:
    ...
    def get_click(self, x, y):
        ...
        if self.upgrade_menu is not None:
            for btn in self.upgrade_menu.get_buttons():
                # if the button is clicked, get the button name
                if btn.clicked(x, y):
                    self.button_response = btn.response()
            # if the button is not clicked, cancel selecting the tower
    ...
    def upgrade(self):
        ...
        if self.selected_tower.level < 5:     # if tower's level is smaller than five
            self.selected_tower.level += 1     # level up
    ...
    def sell(self):
        ...
        self.__towers.remove(self.selected_tower)   # move tower away from self.__towers
        self.upgrade_menu = None                    # reset self.upgrade_menu
        self.selected_tower = None                  # reset self.selected_tower

    def get(self):
        return self.__towers   # return self.__towers
